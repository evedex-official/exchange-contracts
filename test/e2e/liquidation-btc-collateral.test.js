...
const TEST_BTC_CURRENT_PRICE = BTC_CURRENT_PRICE * 10n;
...
describe(flow, () => {
  ...
  it('Alice, Bob, Liquidator deposit btc', async () => {
    const { btcToken, alice, bob, liquidator, depositDex } = await generateSuit(flow);
    const deposit = async ({ user, amount = BTC_DEPOSIT_AMOUNT }) => {
      ...
      await writeContract(user, {
        ...
        abi: depositDex.abi,
        args: [btcToken.address, amount],
      });
    };
    await Promise.all([
      deposit({ user: alice }),
      deposit({ user: bob }),
      deposit({ user: liquidator, amount: BTC_DEPOSIT_AMOUNT * 100n }),
    ]);
  });
  ...
  it('Alice and Bob create opposite orders', async () => {
    ...
    const createOrder = async ({ side, userWallet, userSessionWallet }) => {
      ...
      const orderExt = createOrderExtended({
        collateralIndex: BTC_COLLATERAL_INDEX,
